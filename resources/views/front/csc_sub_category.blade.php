	...
	<div class="container">
		<div class="row">
			...
			<aside>
					<ul>
						...
						<li>
							...
							<a href="{{url('category/'.$category->slug) }}"><div class="text-drop @if($main_category->title == $category->title){{'active'}}@endif ">{{$category->title}}</div></a> 
							<a class="dropdown-btn togglebtn @if($main_category->title == $category->title){{'active-dropbtn'}}@endif">
							...
							<div class="dropdown-container @if($main_category->title == $category->title){{'active-container'}}@endif">
							...
							@else
								...
								@else
								<a href="{{url('category/'.$category->slug.'/'.$sub_cat->slug)  }}"><div class="text-drop @if($sub_cat->title == $data->title){{'active'}}@endif">{{$sub_cat->title}}</div></a>
								<a class="dropdown-btn togglebtn @if($sub_cat->title == $data->title){{'active-dropbtn'}}@endif">
								<i class="fa fa-plus"></i>
								</a>
								<div class="dropdown-container @if($sub_cat->title == $data->title){{'active-container'}}@endif">
			...
			<div class="col-md-9">
				...
				<div class="row">
					<div class="col-md-4">
						<a href="{{url('category/'.$main_category->slug.'/'.$data->slug.'/'.$product->slug) }}">
						<img src="{{ asset('public/images/product/'.$product->image) }}" class="img-responsive ">
					</a>
					</div>
					<div class="col-md-8">
						<h5><a href="{{url('category/'.$main_category->slug.'/'.$data->slug.'/'.$product->slug) }}">{{$product->title}}</a><br>
						</h5>
						<hr class="smallpadd">
						<div class="para-div">
					...
					<a class="readmore access-link">Read More</a>
							<a class="readmore2 access-link now-hide">Read less</a>
						<!-- <a class="access-link" href="{{url('category/'.$data->slug.'/'.$product->slug) }}">More Information  Chevron-right ></a> -->
					</div>
				</div>
				<hr>
				@endforeach
